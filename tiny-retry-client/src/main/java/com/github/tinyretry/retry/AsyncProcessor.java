...
public class AsyncProcessor implements Observer<ProcessResult> {
	 ...
	 *            需要执行的任务的数据
	 * @param callbacks
	 *            回调函数，任务执行结束时回调，仅限第一次调用，超时重试无法调用到
	 * 
	 * @throws RetryRemoteException
	 *             远程服务失败，任务没有记录成功，该异常说明整个都失败了
	 * @throws McJobScheduleException
	 *             注册本地job失败,还可以等待超时处理,可以选择忽略
	 */
	...
	private void register(Task task, Serializable data, long calculateNextTime, Observer<ProcessResult>... callbacks)
		...
		RetryJob job = RetryJobTools.generateMcJob(RetryJobTools.generateMcJobDefinition(
				...
				RetryJobTools.generateJobName(task), task.getDescription()));
		...
		McJobGroup jobGroup = RetryJobTools.generateMcJobGroup(RetryJobTools.generateJobName(task),
				calculateNextTime);
			...
			if (result != null) {
				...
				if (!result.isSuccess()) {
					...
					try {
						...
						taskManage.updateMcTask(mcTaskDO);
					...
					logger.error("fail deal job" + taskId + "-" + job.getTask().getAppCode() + "-"
							+ job.getTask().getTaskName() + "--->"
							+ job.getElapsedTime() + " -- " + job.getActualRunTime() + " -- " + job.successRate());
				} else {
					// 考虑到数据库压力，直接删除
					try {
						taskManage.deleteMcTask(mcTaskDO);
	...
	private Task buildTask(Task task, TaskStatus jobStatus, long calculateNextTime, Serializable data)
		...
		mcTaskDO.setProcessor(RetryJobTools.getIPAddress());
		mcTaskDO = taskManage.insertMcTask(mcTaskDO);
