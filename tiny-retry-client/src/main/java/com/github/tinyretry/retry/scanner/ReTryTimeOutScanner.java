...
public class ReTryTimeOutScanner extends RetryScanner {
	...
	public boolean execute() throws Exception {
		...
		List<McTaskDO> dbTasks = taskManage.queryMcTaskList(generateQuery());
		int dealCount = 0;
		if (dbTasks != null) {
			...
			for (McTaskDO task : dbTasks) {
				if (task != null && server.getTasks().containsKey(task.getType())) {
					...
					if (tempTask != null) {
						...
						long preTime = RetryJobTools
								.calculatePreTime(cloneTask.getNextTime(), cloneTask.getRetryPeriod(),
										cloneTask.getRetryFactor(), cloneTask.getCurrentTime());
						long expirtTime = (cloneTask.getExecuteTimeOut() + preTime);
						if (currentMillisTime > expirtTime) {

							// 3. unregister job
							RetryJob job = RetryJobTools.generateMcJob(RetryJobTools.generateMcJobDefinition(
									RetryJobTools.generateJobName(cloneTask),
									RetryJobTools.generateJobName(cloneTask), cloneTask.getDescription()));
							...
							try {
								...
								update.setNextTime(RetryJobTools.calculateNextTime(cloneTask.getPriority(),
										cloneTask.getRetryFactor(), update.getVersion()));
								update.setOldStatus(TaskStatus.DEALING.getStatus());
								Integer updateMcTask = taskManage.updateMcTaskStatus(update);

								if (updateMcTask > 0) {
									cloneTask.setCurrentTime(task.getVersion());
									cloneTask.setNextTime(task.getNextTime());
									logger.info("Time out task info: {},{},{},{},{}", new Object[] {
											cloneTask.getTaskId(),
											currentMillisTime, preTime, expirtTime,
											ToStringBuilder.reflectionToString(cloneTask) });
									dealCount++;
								}
								// }
							} catch (Exception e) {
		...
		logger.info("ReTryTimeOutScanner running: {} ,dealed {}", dbTasks != null ? String.valueOf(dbTasks.size())
				: "0",
				dealCount);
